        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="panel panel-default">
                    <div class="panel-heading">Edit Stok #{{ $stok->id }}</div>
                            ...
                            <div class="col-md-6">
                                <select class="form-control" name="penerbit_id">
                                    @foreach ($penerbit as $var)
                                        <option value="{{$var->id}}"@if(($stok->penerbit_id)==($var->id)) selected @endif>{{$var->nama}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group {{ $errors->has('buku_id') ? 'has-error' : ''}}">
                            ...
                            <div class="col-md-6">
                                <select class="form-control" name="buku_id">
                                    @foreach ($buku as $var)
                                        <option value="{{$var->id}}"@if(($stok->buku_id)==($var->id)) selected @endif>{{$var->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
